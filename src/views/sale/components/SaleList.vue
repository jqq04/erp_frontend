    ...
    <el-dialog
      ...
      <div style="width: 90%; margin: 0 auto">
        <el-form :model="copySheet" label-width="80px" ref="copySheet">
          <label></label>
          <el-form-item
              v-for="(item, index) in copySheet.saleSheetContent"
              :key="index"
              :label="'商品' + index">
            <div>
              <label>数量 </label>
...
import { writeBackSale } from "../../../network/finance";
export default {
  name: 'SaleList',
  props: {
    list: Array,
  ...
  methods: {
    ...
    writeBack(index){
      ...
      config.saleSheetContent.forEach((item) => {item.quantity = -item.quantity});
      writeBackSale(config).then(res=>{
        this.$emit("refresh")
        this.$message({
          message: '红冲成功!',
          type: 'success'
    ...
    submitForm(){
      ...
      writeBackSale(config).then(res=>{
        ...
        writeBackSale(config1).then(res=>{
          this.$emit("refresh")
          this.$message({
            message: '复制成功!',
            type: 'success'
