    ...
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        <el-row>
          ...
          <el-col :span="6">
            <span><strong>退回总额合计: </strong>{{item.returnsAmount}}(元)</span>
          </el-col>
        </el-row>
...
import { writeBackSaleReturns } from "../../../network/finance";
...
export default {
  ...
  methods: {
    ...
    writeBack(index){
      ...
      config.saleReturnsSheetContent.forEach((item) => {item.quantity = -item.quantity;});
      writeBackSaleReturns(config).then(res=>{
        this.$emit("refresh")
        this.$message({
          message: '红冲成功!',
          type: 'success'
