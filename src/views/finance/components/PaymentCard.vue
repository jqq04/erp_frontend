    ...
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        <el-row>
          ...
          <el-col :span="6">
            <span><strong>付款总额: </strong>{{item.totalAmount}}(元)</span>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
    ...
    <el-dialog
      ...
      <div style="width: 90%; margin: 0 auto">
        <el-form :model="copySheet" label-width="80px" ref="copySheet">
          <label></label>
          <el-form-item
              v-for="(item, index) in copySheet.paymentSheetContent"
              :key="index"
              :label="'账户' + index">
            <div>
              <label>转账金额 </label>
...
<script>
import { approvalPayment} from '../../../network/finance'
import { writeBackPayment } from "../../../network/finance";
...
export default {
  ...
  methods: {
    ...
    approval(id) {
      ...
      if (this.type === 1) {
        approvalPayment(config).then(res => {
          this.$emit("refresh")
          this.$message({
            message: '操作成功!',
            type: 'success'
    ...
    writeBack(index){
      ...
      writeBackPayment(config).then(res=>{
        this.$emit("refresh")
        this.$message({
          message: '红冲成功!',
          type: 'success'
    ...
    submitForm(){
      ...
      writeBackPayment(config).then(res=>{
        ...
        writeBackPayment(config1).then(res=>{
          this.$emit("refresh")
          this.$message({
            message: '复制成功!',
            type: 'success'
