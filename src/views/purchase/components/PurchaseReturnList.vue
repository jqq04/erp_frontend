...
import { writeBackPurchaseReturns } from "../../../network/finance";
...
export default {
  ...
  methods: {
    ...
    writeBack(index){
      ...
      config.purchaseReturnsSheetContent.forEach((item) => {item.quantity = -item.quantity;});
      writeBackPurchaseReturns(config).then(res=>{
        this.$emit("refresh")
        this.$message({
          message: '红冲成功!',
          type: 'success'
